...
app.use(cors()); // CORS middleware for enabling cross requests.
...
app.get("/searchUser/:name", async (req, res) => {
  ...
  // Fetches the GitLab user profile data.
  ...
  if (gitHubProfileData.message === "Not Found") {
    // If GitHub user is "NOT" found it creates a profile object indicating it.
    let gitHubProfile = {
      userFound: false,
      gitHubAvatar: "",
      gitHubUserName: "",
  ...
  // Processes the GitLab profile data.
  if (gitLabProfileData < 1) {
    // If GitLab user is "NOT" found it creates a profile object indicating it.
    let gitLabProfile = {
      userFound: false,
      gitLabAvatar: "",
      gitLabUserName: "",
  ...
  // Responds with appropriate message and the profile data.
  if (profilesData < 1) {
